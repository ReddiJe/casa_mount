...
if ( ! function_exists( 'udesign_get_post_thumb' ) ) {
	...
	function udesign_get_post_thumb( $post_id = null, $remove_thumb_frame = false, $thumb_frame_shadow = false, $post_thumb_width = 60, $post_thumb_height = 60, $alignment = 'alignleft', $thumb_fallback = '' ) {
		...
		if ( true === $thumb_fallback || $thumb_fallback == 1 ) {
			$thumb_fallback =  get_template_directory_uri() . '/assets/images/default-thumb.png';
		}
		...
		if ( function_exists('udesign_get_the_image') ) { 
			...
			echo ( isset( $the_thumb_html_as_array['src'] ) && $the_thumb_html_as_array['src'] ) ? '<div class="small-custom-frame-wrapper ' . $alignment . $shadow_class . '">' . $thumb_frame_wrappers_before . '<a href="' . get_permalink() . '" title="' . the_title( '', '', false ) . '"><img src="' . udesign_process_image( $the_thumb_html_as_array['src'], $post_thumb_width, $post_thumb_height, true, '', false ) . '" width="' . $post_thumb_width . '" height="' . $post_thumb_height . '" alt="' . $the_thumb_html_as_array['alt'] . '" /></a>' . $thumb_frame_wrappers_after . '</div>' : '';
			...
			if ( $default_thumb ) { // Default thumbnail option is selected.
				if ( $the_post_thumbnail_link ) { // Look for the thumbnail passed as a 'post_thumbnail' or 'thumbnail' custom field.
					echo '<div class="small-custom-frame-wrapper ' . $alignment.$shadow_class . '">' . $thumb_frame_wrappers_before . '<a href="' . get_the_permalink() . '" title="' . get_the_title() . '"><img src="' . udesign_process_image( $the_post_thumbnail_link, $post_thumb_width, $post_thumb_height, true, '', false ) . '" width="' . $post_thumb_width . '" height="' . $post_thumb_height . '" alt="' . get_the_title() . '" /></a>' . $thumb_frame_wrappers_after . '</div>';
				} else { // Load default thumbnail image.
					echo '<div class="small-custom-frame-wrapper ' . $alignment.$shadow_class . '">' . $thumb_frame_wrappers_before . '<a href="' . get_the_permalink() . '" title="' . get_the_title() . '"><img src="' . $default_thumb . '" width="' . $post_thumb_width . '" height="' . $post_thumb_height . '" alt="' . get_the_title() . '" /></a>' . $thumb_frame_wrappers_after . '</div>';
				}
			} else { // Default thumbnail option is NOT selected.
				if ( $the_post_thumbnail_link ) { // look for the thumbnail passed as a 'post_thumbnail' or 'thumbnail' custom field.
					echo '<div class="small-custom-frame-wrapper ' . $alignment.$shadow_class . '">' . $thumb_frame_wrappers_before . '<a href="' . get_the_permalink() . '" title="' . get_the_title() . '"><img src="' . udesign_process_image( $the_post_thumbnail_link, $post_thumb_width, $post_thumb_height, true, '', false ) . '" width="' . $post_thumb_width . '" height="' . $post_thumb_height . '" alt="' . get_the_title() . '" /></a>' . $thumb_frame_wrappers_after . '</div>';
