...
function get_portfolio_item_thumbnail( $curr_post_ID, $num_cols='3', $width='260', $height='160', $do_not_link_adjacent_items = 'no', $echo = true ) {
	...
	$portfolio_default_thumb = get_template_directory_uri() . '/assets/images/portfolio-default-thumb.jpg';
	...
	$preview_item_title = do_shortcode( esc_html( get_post_meta($curr_post_ID, 'portfolio_item_preview_title', true) ) ); // Grab the preview item title the custom field 'portfolio_item_preview_title', if set.

	if ( function_exists('udesign_get_the_image') ) { // The case when "Get The Image" plugin is available.
		...
		if ( $preview_item ) { // If preview item is available, go ahead and generate the thumbnail as a link.
			...
			$output .= '<span class="portfolio-zoom-image-'.$num_cols.'-col"> </span>';
			$output .= '<img class="hover-opacity" src="'.udesign_process_image( $portfolio_thumb_as_array['src'], $width, $height, true, $portfolio_item_thumb_crop_align, $portfolio_item_thumb_retina ).'" width="'.$width.'" height="'.$height.'" alt="'.esc_attr__('Preview item is not available!', 'u-design').'" />';
		}
		...
		if( $portfolio_item_thumb ) { // Thumbnail is provided.
			if ( $preview_item ) { // If preview item is available, go ahead an link it to the thumbnail.
				...
				$output .= '<span class="portfolio-zoom-image-'.$num_cols.'-col"> </span>';
				$output .= '<img class="hover-opacity" src="'.udesign_process_image( $portfolio_item_thumb, $width, $height, true, $portfolio_item_thumb_crop_align, $portfolio_item_thumb_retina ).'" width="'.$width.'" height="'.$height.'" alt="'.esc_attr__('Preview image not available!', 'u-design').'" />';
			}
			...
			$output .= '<span class="portfolio-zoom-image-'.$num_cols.'-col"> </span>';
			$output .= '<img class="hover-opacity" src="'.udesign_process_image( $portfolio_default_thumb, $width, $height, true, $portfolio_item_thumb_crop_align, $portfolio_item_thumb_retina ).'" alt="'.esc_attr__("Default Image", 'u-design').'" width="'.$width.'" height="'.$height.'" />';
		}
	}
...
/**
* Get image attachments from a post. Return the images as HTML links with 'rel' attribute added to them so that prettyPhoto can use them.
