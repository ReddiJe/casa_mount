...
define( 'UDESIGN_VERSION', '3.3.5' );


if ( ! function_exists( 'who_can_edit_udesign_theme_options' ) ) {
	
	/**
	 * Use this function for access control role capability: 'manage_options'.
	 * By default only admins can edit and see the theme's options with role capability: 'manage_options'.
	 * More on roles and capabilities: https://codex.wordpress.org/Roles_and_Capabilities
	 * 
...
if ( is_admin() && isset( $_GET['activated'] ) && $pagenow == 'themes.php' ) {
    wp_safe_redirect( admin_url( 'admin.php?page=udesign_options_page' ) );
    exit;
}
...
if ( function_exists( 'load_theme_textdomain' ) ) {
	load_theme_textdomain( 'u-design', get_template_directory() . '/lang' );
}

// U-Design action hook functions.
include( trailingslashit( get_template_directory() ) . 'inc/admin/u-design-hooks/u-design-theme-hooks.php' );
...
function register_post_types(){
	...
	register_post_type( 'News', [
		...
		'labels' => [
			...
			'add_new_item'       => 'Add Title', // заголовка у вновь создаваемой записи в админ-панели.
			'edit_item'          => 'Edit', // для редактирования типа записи
			'new_item'           => 'News Text', // текст новой записи
	...
	register_post_type( 'Testimonials', [
		...
		'labels' => [
			...
			'add_new'            => 'Add Testimonials', // для добавления новой записи
			'add_new_item'       => 'Add Title', // заголовка у вновь создаваемой записи в админ-панели.
			'edit_item'          => 'Edit', // для редактирования типа записи
			'new_item'           => 'Testimonials Text', // текст новой записи
			'view_item'          => 'View Testimonials', // для просмотра записи этого типа.
			'search_items'       => 'Search', // для поиска по этим типам записи
			'not_found'          => 'Not found', // если в результате поиска ничего не было найдено
			'not_found_in_trash' => 'Not found', // если не было найдено в корзине
			'parent_item_colon'  => '', // для родителей (у древовидных типов)
...
function udesign_init_styles() {
	...
	wp_enqueue_style('u-design-reset', get_template_directory_uri() . '/assets/css/frontend/layout/reset.css', false, '1.0', 'screen');
	wp_enqueue_style('u-design-grid-960', get_template_directory_uri() . '/assets/css/frontend/layout/960.css', false, '1.0', 'screen');
	wp_enqueue_style('u-design-superfish_menu', get_template_directory_uri() . '/inc/frontend/superfish-menu/css/superfish.css', false, '1.7.2', 'screen');
        if ( $udesign_options['enable_prettyPhoto_script'] ) {
            wp_enqueue_style('u-design-pretty_photo', get_template_directory_uri() . '/inc/frontend/prettyPhoto/css/prettyPhoto.css', false, '3.1.6', 'screen');
        }
        
        // Load Font Awesome styles if enabled.
        if ( ! $udesign_font_awesome_options['udesign_disable_font_awesome'] ) {
		wp_enqueue_style('u-design-font-awesome', get_template_directory_uri(). '/inc/shared/font-awesome/css/font-awesome.min.css', false, UDESIGN_VERSION, 'screen');
	    
		// Add 'udesign-fa' class to body tag.
		function udesign_add_udesign_font_awesome_body_class( $classes ) {
			// Add 'udesign-fa' to the $classes array.
			$classes[] = 'udesign-fa';
			return $classes;
		}
		add_filter( 'body_class', 'udesign_add_udesign_font_awesome_body_class' );
        }
	...
	wp_enqueue_style('u-design-style', get_template_directory_uri() . "/assets/css/frontend/global/style.css", false, UDESIGN_VERSION, 'screen');
		...
		if ( get_theme_mod( 'udesign_custom_style_css_updated_from_db' ) ) {
			$custom_style_css = get_template_directory(). '/assets/css/frontend/global/custom_style.css';
		...
		wp_enqueue_style('u-design-custom-style', get_template_directory_uri() . '/assets/css/frontend/global/custom_style.css', false, UDESIGN_VERSION.$rand_ver, 'screen');
        } else { // otherwise use "custom_style.php" file
		wp_enqueue_style('u-design-custom-style', get_template_directory_uri() . '/assets/css/frontend/global/custom_style.php', false, UDESIGN_VERSION, 'screen');
        }
        
        if ( $udesign_responsive ) {
            wp_enqueue_style('u-design-responsive', get_template_directory_uri() . '/assets/css/frontend/layout/responsive.css', false, UDESIGN_VERSION, 'screen');
        }
		...
		wp_enqueue_style('u-design-fluid', get_template_directory_uri() . '/assets/css/frontend/layout/fluid.css', false, UDESIGN_VERSION, 'screen');
        }
        
        // This option makes sure both the "parent" or "child" themes' "style.css" files are not needlessly being loaded.
        if ( $udesign_options['enable_default_style_css'] ) {
		$udesign_style_css_file = get_stylesheet_directory(). '/style.css'; // Will point to parent's or child's style.css, whichever is currently active.
		if ( file_exists( $udesign_style_css_file ) ) {
			// Set cache buster file version.
			$style_css_last_modified = filemtime( $udesign_style_css_file );
			if ( get_theme_mod( 'udesign_theme_style_last_modified' ) && get_theme_mod( 'udesign_theme_style_last_modified' ) !== $style_css_last_modified ) {
				remove_theme_mod( 'udesign_theme_style_last_modified' );
			} else {
				set_theme_mod( 'udesign_theme_style_last_modified', $style_css_last_modified );
			}
			// Assign the last modified datestamp to be appended to the file version.
			$style_rand_ver = ( $style_css_last_modified ) ? '.' . $style_css_last_modified : '';

			// Enqueue the style.css file.
			wp_enqueue_style( 'u-design-style-orig', get_stylesheet_directory_uri() . "/style.css", false, UDESIGN_VERSION . $style_rand_ver, 'screen' );
		}
        }
        
        // Load the Internet Explorer 9 or less specific stylesheet
	wp_enqueue_style( 'u-design-ie9', get_template_directory_uri() . '/assets/css/frontend/layout/ie-all.css', array( 'u-design-style' ), UDESIGN_VERSION, 'screen' );
...
function udesign_init_scripts() {
	...
	if( is_front_page() && $current_slider == '4' ) {
	    wp_enqueue_script('cycle', get_template_directory_uri()."/inc/frontend/sliders/cycle/jquery.cycle.all.min.js", array('jquery'), '3.0.3', true);
	    wp_enqueue_script('cycle1', get_template_directory_uri()."/inc/frontend/sliders/cycle/cycle1/cycle1_script.js", array('jquery'), '1.0.1', true);
            ...
            $cycle1_params = array(
                    ...
                    'sync'	=> ( isset( $udesign_options['c1_sync'] ) && $udesign_options['c1_sync'] ) ? 1 : 0
            );
	...
	if( is_front_page() && $current_slider == '5' ) {
	    wp_enqueue_script('cycle', get_template_directory_uri()."/inc/frontend/sliders/cycle/jquery.cycle.all.min.js", array('jquery'), '3.0.3', true);
	    wp_enqueue_script('cycle2', get_template_directory_uri()."/inc/frontend/sliders/cycle/cycle2/cycle2_script.js", array('jquery'), '1.0.1', true);
            ...
            $cycle2_params = array(
                    ...
                    'sync'	=> ( isset( $udesign_options['c2_sync'] ) && $udesign_options['c2_sync'] ) ? 1 : 0,
                    'texttrans'	=> ( isset( $udesign_options['c2_text_transition_on'] ) && $udesign_options['c2_text_transition_on'] ) ? 1 : 0
            );
	...
	if( is_front_page() && $current_slider == '6' ) {
	    wp_enqueue_script('cycle', get_template_directory_uri()."/inc/frontend/sliders/cycle/jquery.cycle.all.min.js", array('jquery'), '3.0.3', true);
	    wp_enqueue_script('jquery-easing', get_template_directory_uri()."/inc/frontend/sliders/cycle/jquery.easing.1.3.js", array('jquery'), '1.3', true);
	    wp_enqueue_script('cycle3', get_template_directory_uri()."/inc/frontend/sliders/cycle/cycle3/cycle3_script.js", array('jquery'), '1.0.1', true);
            // Generate an array of Cycle 1 parameters and pass them to "cycle3_script.js":
            $cycle3_params = array(
                    'timeout'	=> $udesign_options['c3_timeout'],
                    'autostop'	=> ( isset( $udesign_options['c3_autostop'] ) && $udesign_options['c3_autostop'] ) ? 1 : 0
            );
	...
	// PrettyPhoto script.
	if( $udesign_options['enable_prettyPhoto_script'] ) {
            wp_enqueue_script('pretty-photo-lib', get_template_directory_uri()."/inc/frontend/prettyPhoto/js/jquery.prettyPhoto.js", array('jquery'), '3.1.6', true);
            wp_enqueue_script('pretty-photo-custom-params', get_template_directory_uri()."/inc/frontend/prettyPhoto/custom_params.js", array('pretty-photo-lib'), '3.1.6', true);
	...
	// jQuery validation script.
        if ( is_page_template('page-Contact.php') ) {
            wp_enqueue_script('jquery_validate_lib', get_template_directory_uri()."/inc/frontend/jquery-validate/jquery.validate.min.js", array('jquery'), '1.11.1', true);
            wp_enqueue_script('masked_input_plugin', get_template_directory_uri()."/inc/frontend/masked-input-plugin/jquery.maskedinput.min.js", array('jquery'), '1.3.1', true);
        }
	...
	// Isotope related Sortable Portfolio scripts.
        if ( is_page_template('page-Portfolio1ColSortable.php') || is_page_template('page-Portfolio2ColSortable.php') || is_page_template('page-Portfolio3ColSortable.php') || is_page_template('page-Portfolio4ColSortable.php') ) {
            wp_enqueue_script('jquery-isotope-lib', get_template_directory_uri()."/inc/frontend/isotope/jquery.isotope.min.js", array('jquery'), '1.5.25', true);
            wp_enqueue_script('isotope-custom-scripts', get_template_directory_uri()."/inc/frontend/isotope/isotope-custom-scripts.js", array('jquery-isotope-lib'), '1.0.0', true);
        }
        
	// Superfish Dropdown menu scripts (combined).
	wp_enqueue_script('superfish-menu', get_template_directory_uri()."/inc/frontend/superfish-menu/js/superfish.combined.js", array('jquery'), '1.7.2', true);
	...
	wp_enqueue_script('udesign-scripts', get_template_directory_uri()."/assets/js/frontend/script.js", array('jquery'), '1.0', true);
        wp_localize_script('udesign-scripts', 'udesign_script_vars', array(
                    'search_widget_placeholder' => __('Type here to search', 'u-design'),
        ...
        if ( $udesign_responsive ) {
            if ( $udesign_options['responsive_menu'] == 'responsive_menu_2') {
                ...
                wp_enqueue_script('udesign-responsive-menu-2', get_template_directory_uri() . '/inc/frontend/meanmenu/jquery.meanmenu.min.js', '', '2.0.8', true);
                wp_enqueue_script('udesign-responsive-menu-2-options', get_template_directory_uri() . '/inc/frontend/meanmenu/jquery.meanmenu.options.js', '', '2.0.8', true);
                ...
                wp_enqueue_script('udesign-responsive-menu-1', get_template_directory_uri() . '/inc/frontend/selectnav/selectnav.min.js', '', '0.1', true);
                wp_enqueue_script('udesign-responsive-menu-1-options', get_template_directory_uri() . '/inc/frontend/selectnav/selectnav-options.js', '', '0.1', true);
                wp_localize_script('udesign-responsive-menu-1-options', 'udesign_selectnav_vars', array(
                            'selectnav_menu_label' => __('Navigation', 'u-design')
                        )
                );
            }
        }
	...
	if ( is_singular() && comments_open() && get_option('thread_comments') ) {
        ...
        /**
         * Load only if WordPress 4.2 or later as the 'wp_script_add_data' was added in WP 4.2.
         */
        global $wp_version;
        if ( version_compare( $wp_version, '4.2.0', '>=' ) ) {
            // Load 'Respond.js' - a fast & lightweight polyfill for min/max-width CSS3 Media Queries (for IE 6-8)
            wp_register_script( 'u-design-respond', get_template_directory_uri() . '/assets/js/frontend/respond.min.js', array(), '1.4.2' );
...
register_nav_menus( array(
	'primary' => esc_html__( 'Primary Navigation', 'u-design' ),
	'footer-bottom' => _('Footer Menu')
) );

if ( ! function_exists( 'udesign_nav' ) ) {
...
function translatable_search_form( $form ) {
    ...
    <div><label class="screen-reader-text" for="search-field">' . __('Search for:', 'u-design') . '</label>
	<input type="text" value="' . get_search_query() . '" name="s" id="search-field" />
	<input type="submit" id="searchsubmit" value="'. esc_attr__('Search', 'u-design') .'" />
    </div>
    </form>';
    return $form;
}
...
function get_image_url() {
	...
	$first_img = get_template_directory_uri() . '/assets/images/default-thumb.png';
    }
    return $first_img;
}
...
function get_category_parents_for_portfolio_page( $id, $link = false, $separator = '/', $nicename = false, $portfolio_page_id='', $visited = array() ) {
	...
	if ( $parent->parent && ( $parent->parent != $parent->term_id ) && !in_array( $parent->parent, $visited ) ) {
		...
		$chain .= '<a href="'.get_permalink( $portfolio_page_id ).'" title="'.esc_attr( sprintf( __( "Go back to %s", 'u-design' ), get_the_title($portfolio_page_id) ) ).'">'.get_the_title($portfolio_page_id).'</a>' . $separator . ' ';
	}
	if ( $link ) { // generate comma separated list of categories' links that the current single post has been assigned to
		...
		foreach((get_the_category()) as $category) {
			...
			if ( preg_match( '/\?/', get_permalink($portfolio_page_id) ) ) $query_string_prefix = '&amp;';
                        $curr_cat_link = '<a href="'.get_permalink($portfolio_page_id).$query_string_prefix.'cat=' . ( $category->term_id ) . '" title="' . esc_attr( sprintf( __( "Go back to %s", 'u-design' ), $category->cat_name ) ) . '">'.$category->cat_name.'</a>';
...
<?php                   printf(__('<span class="the-comment-time-and-date"><span class="time">%1$s</span> on <a class="comment-date" href="#comment-%2$s" title="%3$s">%3$s</a></span>', 'u-design'), get_comment_time(__('g:i a','u-design')), get_comment_ID(), get_comment_date(__('F j, Y', 'u-design')) );
                        edit_comment_link(esc_html__('edit', 'u-design'),'&nbsp;&nbsp;',''); ?>
                    </div>
                </div>

                <div class="commenttext">
<?php               if ($comment->comment_approved == '0') : ?>
                        <em><?php esc_html_e('Your comment is awaiting moderation.', 'u-design') ?></em>
                        <br />
<?php               endif; ?>
<?php               comment_text() ?>
                    <div class="reply">
 ...
 */
include ( trailingslashit( get_template_directory() ) . 'inc/admin/u-design-get-the-image/get-the-image.php' );

/**
 * Include U-Design post thumbnail generating function
 */
include ( trailingslashit( get_template_directory() ) . 'inc/frontend/post-thumbnail.php' );
...
function udesign_filter_widget( $params ) {
...
add_filter( 'dynamic_sidebar_params','udesign_filter_widget' );

if ( function_exists('register_sidebar') ) {
	register_sidebar(array(
		'name' => 'Pages Sidebar',
                'id'          => 'sidebar-1',
		'description' => esc_html__('A widget area, used as a sidebar for regular pages.', 'u-design'),
	...
	register_sidebar(array(
		'name' => 'PortfolioSidebar',
                'id'          => 'sidebar-2',
		'description' => esc_html__('A widget area, used as a sidebar for the Portfolio section.', 'u-design'),
		'before_widget' => '<div id="%1$s" class="widget %2$s substitute_widget_class">',
		'after_widget' => '</div>',
		'before_title' => '<h3 class="widgettitle">',
		'after_title' => '</h3>',
	...
	register_sidebar(array(
		'name' => 'BlogSidebar',
                'id'          => 'sidebar-3',
		'description' => esc_html__('A widget area, used as a sidebar for the Blog/News section.', 'u-design'),
		'before_widget' => '<div id="%1$s" class="widget %2$s substitute_widget_class">',
		'after_widget' => '</div>',
		'before_title' => '<h3 class="widgettitle">',
		'after_title' => '</h3>',
	...
	register_sidebar(array(
		'name' => 'ContactSidebar',
                'id'          => 'sidebar-4',
		'description' => esc_html__('A widget area, used as a sidebar for the Contact page.', 'u-design'),
		'before_widget' => '<div id="%1$s" class="widget %2$s substitute_widget_class">',
		'after_widget' => '</div>',
		'before_title' => '<h3 class="widgettitle">',
		'after_title' => '</h3>',
	...
	register_sidebar(array(
		'name' => 'SitemapSidebar',
                'id'          => 'sidebar-5',
		'description' => esc_html__('A widget area, used as a sidebar for the Sitemap page.', 'u-design'),
		'before_widget' => '<div id="%1$s" class="widget %2$s substitute_widget_class">',
		'after_widget' => '</div>',
		'before_title' => '<h3 class="widgettitle">',
		'after_title' => '</h3>',
	...
	register_sidebar(array(
		'name' => esc_html__('Home Page Before Content', 'u-design'),
		'id' => 'home-page-before-content',
		'description' => esc_html__('A widget area positioned just above the Home Page Main Content area.', 'u-design'),
	...
	register_sidebar(array(
		'name' => esc_html__('Home Page Column 1', 'u-design'),
		'id' => 'home-page-column-1',
		'description' => esc_html__('A widget area, used as the 1st column in the Main Content area.', 'u-design'),
		'before_widget' => '<div class="cont_col_1 %2$s substitute_widget_class">',
		'after_widget' => '</div>',
	...
	register_sidebar(array(
		'name' => esc_html__('Home Page Column 2', 'u-design'),
		'id' => 'home-page-column-2',
		'description' => esc_html__('A widget area, used as the 2nd column in the Main Content area.', 'u-design'),
	...
	register_sidebar(array(
		'name' => esc_html__('Home Page Column 3', 'u-design'),
		'id' => 'home-page-column-3',
		'description' => esc_html__('A widget area, used as the 3rd column in the Main Content area.', 'u-design'),
	...
	register_sidebar(array(
		'name' => esc_html__('Home Page Column 4', 'u-design'),
		'id' => 'home-page-column-4',
		'description' => esc_html__('A widget area, used as the 4th column in the Main Content area.', 'u-design'),
	...
	register_sidebar(array(
		'name' => esc_html__('Home Page After Content Row 1', 'u-design'),
		'id' => 'home-page-after-content-row-1',
		'description' => esc_html__('A widget area positioned just below the Home Page Main Content area.', 'u-design'),
	...
	register_sidebar(array(
		'name' => esc_html__('Home Page After Content Row 2', 'u-design'),
		'id' => 'home-page-after-content-row-2',
		'description' => esc_html__('A widget area positioned just above the Bottom Widget area.', 'u-design'),
	...
	// Bottom Widget Areas
	register_sidebar(array(
		'name' => esc_html__('Bottom 1', 'u-design'),
		'id' => 'bottom-widget-area-1',
		'description' => esc_html__('A widget area, used as the 1st column in the Bottom area (just above the footer).', 'u-design'),
	...
	register_sidebar(array(
		'name' => esc_html__('Bottom 2', 'u-design'),
		'id' => 'bottom-widget-area-2',
		'description' => esc_html__('A widget area, used as the 2nd column in the Bottom area (just above the footer).', 'u-design'),
		'before_widget' => '<div class="bottom-col-content %2$s substitute_widget_class">',
		'before_title' => '<h3 class="bottom-col-title">',
		'after_title' => '</h3>',
		'after_widget' => '</div>',
	...
	register_sidebar(array(
		'name' => esc_html__('Bottom 3', 'u-design'),
		'id' => 'bottom-widget-area-3',
		'description' => esc_html__('A widget area, used as the 3rd column in the Bottom area (just above the footer).', 'u-design'),
		'before_widget' => '<div class="bottom-col-content %2$s substitute_widget_class">',
		'before_title' => '<h3 class="bottom-col-title">',
		'after_title' => '</h3>',
		'after_widget' => '</div>',
	...
	register_sidebar(array(
                'name' => esc_html__('Bottom 4', 'u-design'),
                'id' => 'bottom-widget-area-4',
                'description' => esc_html__('A widget area, used as the 4th column in the Bottom area (just above the footer).', 'u-design'),
                'before_widget' => '<div class="bottom-col-content %2$s substitute_widget_class">',
                'before_title' => '<h3 class="bottom-col-title">',
                'after_title' => '</h3>',
                'after_widget' => '</div>',
	...
	register_sidebar(array(
		'name' => 'PagesSidebar2',
                'id'          => 'sidebar-17',
		'description' => esc_html__('A widget area, used as a sidebar for the Page Template 2.', 'u-design'),
		'before_widget' => '<div id="%1$s" class="widget %2$s substitute_widget_class">',
		'after_widget' => '</div>',
		'before_title' => '<h3 class="widgettitle">',
		'after_title' => '</h3>',
	...
	register_sidebar(array(
		'name' => 'PagesSidebar3',
                'id'          => 'sidebar-18',
		'description' => esc_html__('A widget area, used as a sidebar for the Page Template 3.', 'u-design'),
		'before_widget' => '<div id="%1$s" class="widget %2$s substitute_widget_class">',
		'after_widget' => '</div>',
		'before_title' => '<h3 class="widgettitle">',
		'after_title' => '</h3>',
	...
	register_sidebar(array(
		'name' => 'PagesSidebar4',
                'id'          => 'sidebar-19',
		'description' => esc_html__('A widget area, used as a sidebar for the Page Template 4.', 'u-design'),
		'before_widget' => '<div id="%1$s" class="widget %2$s substitute_widget_class">',
		'after_widget' => '</div>',
		'before_title' => '<h3 class="widgettitle">',
		'after_title' => '</h3>',
	...
	register_sidebar(array(
		'name' => 'PagesSidebar5',
                'id'          => 'sidebar-20',
		'description' => esc_html__('A widget area, used as a sidebar for the Page Template 5.', 'u-design'),
		'before_widget' => '<div id="%1$s" class="widget %2$s substitute_widget_class">',
		'after_widget' => '</div>',
		'before_title' => '<h3 class="widgettitle">',
		'after_title' => '</h3>',
	...
	register_sidebar(array(
		'name' => 'PagesSidebar6',
                'id'          => 'sidebar-21',
		'description' => esc_html__('A widget area, used as a sidebar for the Page Template 6.', 'u-design'),
		'before_widget' => '<div id="%1$s" class="widget %2$s substitute_widget_class">',
		'after_widget' => '</div>',
		'before_title' => '<h3 class="widgettitle">',
		'after_title' => '</h3>',
	...
	register_sidebar(array(
		'name' => 'PagesSidebar7',
                'id'          => 'sidebar-22',
		'description' => esc_html__('A widget area, used as a sidebar for the Page Template 7.', 'u-design'),
		'before_widget' => '<div id="%1$s" class="widget %2$s substitute_widget_class">',
		'after_widget' => '</div>',
		'before_title' => '<h3 class="widgettitle">',
		'after_title' => '</h3>',
	...
	register_sidebar(array(
		'name' => 'PagesSidebar8',
                'id'          => 'sidebar-23',
		'description' => esc_html__('A widget area, used as a sidebar for the Page Template 8.', 'u-design'),
		'before_widget' => '<div id="%1$s" class="widget %2$s substitute_widget_class">',
		'after_widget' => '</div>',
		'before_title' => '<h3 class="widgettitle">',
		'after_title' => '</h3>',
	...
	register_sidebar(array(
		'name' => esc_html__('Top Area Social Media', 'u-design'),
		'id' => 'top-area-social-media',
		'description' => esc_html__('A widget area positioned in the top right corner of the site designated for social media links and icons.', 'u-design'),
...
function udesign_get_dynamic_sidebar( $index = '' ) {
	$sidebar_contents = "";
	if ( function_exists( 'dynamic_sidebar' ) && is_active_sidebar( $index ) ) {
		ob_start();
...
/* Load any pages and libs only need be loaded for the admin area. */
if ( is_admin() ) {
	
	if ( current_user_can( 'install_themes' ) ) {
		/* Load Theme Notifier. */
		if ( !$udesign_options['disable_the_theme_update_notifier'] == 'yes' ) {
		    require_once( trailingslashit( get_template_directory() ) . 'inc/admin/notifier/update-notifier.php' );
		}
		/* Load the script to register required plugins for U-Design. */
		require_once( trailingslashit( get_template_directory() ) . 'inc/admin/plugin-activation/register-required-plugins.php' );
	}
	...
	/* Load the U-Design 'Import Demo Data' Page. */
	require_once( trailingslashit( get_template_directory() ) . 'udesign-demo-import-page.php' );
	
	/* Load U-Design - Merlin importer. */
	include( trailingslashit( get_template_directory() ) . 'inc/admin/u-design-merlin/u-design-merlin.php' );
	
}

/* Load breadcrumbs script */
if ( isset( $udesign_options['show_breadcrumbs'] ) && $udesign_options['show_breadcrumbs'] == 'yes') {
    include( trailingslashit( get_template_directory() ) . 'inc/frontend/breadcrumbs.php' );
}

/* Load Portfolio Related Function */
require_once( trailingslashit( get_template_directory() ) . 'inc/frontend/portfolio-item-thumbnail.php' );

// U-Design Schema.org Stuff
if ( $udesign_options['enable_udesign_schema_tags'] == 'yes' ) {
    include( trailingslashit( get_template_directory() ) . 'inc/admin/structured-data/schema/u-design-schema.php' );
}
...
if ( !function_exists('udesign_process_image') ) {
	...
	global $udesign_options;
        require_once( trailingslashit( get_template_directory() ) . 'inc/frontend/image-resizer.php' );
...
if ( !function_exists('udesign_page_title') ) {
	...
	$page_title_position = 'default_position';
...
<?php                           else : ?>
                                    <h1 class="category-pagetitle"><?php printf( __('Archive for the <em>%s</em> Category', 'u-design' ), single_cat_title("", false) ); ?></h1>
<?php                           endif; ?>
<?php                       elseif (is_search()) : /* If this is a search results page */ ?>
                                <h1 class="search-pagetitle"><?php printf( __('Search Results for <em>%s</em>', 'u-design' ), get_search_query() ); ?></h1>
<?php                       elseif (is_404()) : /* If this is a 404 page */ ?>
                                <h1 class="404-pagetitle"><?php esc_html_e('Page Not Found (Error 404)', 'u-design'); ?></h1>
<?php                       elseif( is_tag() ) : /* If this is a tag archive */ ?>
                                <h1 class="tag-pagetitle"><?php printf( __('Posts Tagged <em>%s</em>', 'u-design' ), single_tag_title("", false) ); ?></h1>
<?php                       elseif (is_day()) : /* If this is a daily archive */ ?>
                                <h1 class="daily-archive-pagetitle"><?php printf( __('Daily archive for %s', 'u-design' ), get_the_date() ); ?></h1>
<?php                       elseif (is_month()) : /* If this is a monthly archive */ ?>
                                <h1 class="monthly-archive-pagetitle"><?php printf( __('Monthly archive for %s', 'u-design' ), get_the_time('F Y') ); ?></h1>
<?php                       elseif (is_year()) : /* If this is a yearly archive */ ?>
                                <h1 class="yearly-archive-pagetitle"><?php printf( __('Yearly archive for %s', 'u-design' ), get_the_time('Y') ); ?></h1>
<?php                       elseif (is_author()) : /* If this is an author archive */ ?>
                                <h1 class="author-pagetitle"><?php esc_html_e('Author Archive', 'u-design'); ?></h1>
<?php                       elseif (isset($_GET['paged']) && !empty($_GET['paged'])) : /* If this is a paged archive */ ?>
                                <h1 class="paged-archive-pagetitle"><?php esc_html_e('Blog Archives', 'u-design'); ?></h1>
...
function udesign_set_page_title_globally() {
	...
	switch ( udesign_get_current_page_title_position_option( $post_id ) ) {
...
function udesign_add_title_description( $udesign_page_title_html ) {
    global $post;
    $udesign_title_description = '';
    $post_id = ( is_object( $post ) ) ? $post->ID : get_the_ID();
    if ( $post_id ) {
	$udesign_title_description = get_post_meta( $post_id, 'udesign_title_description', true );
    }
...
function udesign_search_form( $form ) {
	...
	<form role="search" method="get" id="searchform" class="searchform" action="<?php echo home_url( '/' ); ?>" >
            <div>
                <label class="screen-reader-text" for="search-field"><?php _e( 'Search for:', 'u-design' ); ?></label>
                <input type="text" id="search-field" name="s" value="<?php echo get_search_query(); ?>" placeholder="<?php _e( 'Type here to search', 'u-design' ); ?>">
                <input type="submit" id="searchsubmit" value="<?php esc_attr_e( 'Search', 'u-design' ); ?>" />
            </div>
	</form>
        <?php
        $form = ob_get_clean();
...
if ( $udesign_options['enable_feedback'] ) {
    function udesign_feedback_button() {
        global $udesign_options;
        ob_start();
		if ( isset( $udesign_options['feedback_btn_text'] ) && $udesign_options['feedback_btn_text'] ) :
			?>
			<div id="feedback"><a href="<?php echo esc_url( $udesign_options['feedback_url'] ); ?>" class="feedback"><?php echo esc_attr( $udesign_options['feedback_btn_text'] ); ?></a></div>
			<?php
		endif;
...
if ( $udesign_options['enable_page_peel'] ) {
    function udesign_enable_page_peel() {
        ...
        ob_start(); ?>
            <div id="page-peel">
                    <a href="<?php echo esc_url( $udesign_options['page_peel_url'] ); ?>" title="<?php esc_attr_e( 'Subscribe', 'u-design' ); ?>"><img src="<?php echo esc_url( get_template_directory_uri() ); ?>/assets/images/page_peel.png" alt="<?php esc_attr_e( 'Subscribe', 'u-design' ); ?>" /></a>
			...
			<div id="search" class="grid_6 prefix_18">
				<form action="<?php echo esc_url( home_url() ); ?>/" method="get">
					<div class="search_box">
						...
						<input id="search-field" name="s" type="text" class="inputbox_focus blur" value="<?php esc_attr_e('Search...', 'u-design'); ?>" />
						<input type="submit"  value="search" class="search-btn" />
					</div>
				</form>
			</div><!-- end search -->
			<?php 
		$searchbox_html = ob_get_clean();
...
if ( ! $udesign_options['fixed_menu_logo_disabled'] ) {
    function udesign_sticky_menu_logo() {
        ...
        if ( $udesign_options['fixed_menu_logo'] ) {
            ...
            $logo_img_url = get_template_directory_uri().'/assets/images/logo.png';
        } ?>
        <a id="sticky-menu-logo" href="<?php echo home_url(); ?>" title="<?php bloginfo('name'); ?>"><img height="40" src="<?php echo esc_url( $logo_img_url ); ?>" alt="logo" /></a>
        <?php
    }
...
function udesign_display_breadcrumbs() {
    ...
    if ( isset($udesign_options['show_breadcrumbs']) && $udesign_options['show_breadcrumbs'] == 'yes' && ! $disable_breadcrumbs ) {
        $wrap_before = '<nav id="breadcrumbs-container" class="container_24">';
...
function udesign_nextpage_links() {
    if( function_exists('wp_pagenavi') ) {
        wp_pagenavi( array( 'wrapper_tag' => 'nav', 'type' => 'multipart' ) );
    } else {
...
function udesign_single_post_image() {
	...
	ob_start();
		...
		if( $udesign_options['display_post_image_in_single_post'] == 'yes' ) {
			display_post_image_fn( get_the_ID(), false );
		}
	$post_image_html = ob_get_clean();
...
function udesign_single_postmetadata() {
	...
	// Insert the single posts "postmetadata" block.
	get_template_part( 'template-parts/post/postmetadata', 'single' );
...
function udesign_single_portfolio_postmetadata() {
	...
	// Insert portfolio's single posts "postmetadata" block.
	if ( $udesign_options['show_portfolio_postmetadata'] == 'yes' ) {
		get_template_part( 'template-parts/post/postmetadata', 'single-portfolio' );
	}
...
function udesign_single_portfolio_entry_navigation() {
	...
	?>
		<nav class="single-post-nav-links">
			<?php if ( $udesign_options['show_portfolio_postmetadata'] != 'yes' || $udesign_options['udesign_single_portfolio_postmetadata_location'] == 'aligntop' ) : ?>
				<div class="single-post-nav-links-divider-before"><?php echo do_shortcode( '[divider]' ); ?></div>
			<?php endif; ?>
			<?php previous_post_link( '&larr; %link', '%title', true ); ?>
			<?php next_post_link( '%link &rarr;', '%title', true ); ?>
		...
		if ( is_a( $next_post , 'WP_Post' ) ) : 
			...
			<div class="next-link-column one_half last_column">
				<?php if ( $next_post_thumb ) : ?>
					<?php $has_post_thumb_class = ' has-post-thumb'; ?>
					<div class="next-post-thumb"><?php echo wp_kses_post( $next_post_thumb ); ?></div>
				<?php endif; ?>
				<div class="next-post-links<?php echo esc_attr( $has_post_thumb_class ); ?>">
					<a rel="next" title="<?php _e( 'Go to next post', 'u-design' ); ?>" href="<?php echo esc_url( $next_post_permalink ); ?>"><?php _e( 'Next Post', 'u-design' ); ?> &nbsp;<i class="fa fa-angle-right" style="font-size: 14px;"></i></a>
					<div class="next-title"><a rel="next" title="<?php echo esc_attr( $title ); ?>" href="<?php echo esc_url( $next_post_permalink ); ?>"><?php echo esc_attr( $title ); ?></a></div>
				</div>
			</div>
			<?php
		endif;
		...
		if ( is_a( $previous_post , 'WP_Post' ) ) : ?>
			<div class="previous-link-column one_half">
				<?php if ( $previous_post_thumb ) : ?>
					<?php $has_post_thumb_class = ' has-post-thumb'; ?>
					<div class="previous-post-thumb"><?php echo wp_kses_post( $previous_post_thumb ); ?></div>
				<?php endif; ?>
				<div class="previous-post-links<?php echo esc_attr( $has_post_thumb_class ); ?>">
					<a rel="previous" title="<?php _e( 'Go to previous post', 'u-design' ); ?>" href="<?php echo esc_url( $previous_post_permalink ); ?>"><i class="fa fa-angle-left" style="font-size: 14px;"></i> &nbsp;<?php _e( 'Previous Post', 'u-design' ); ?></a>
					<div class="prev-title"><a rel="previous" title="<?php echo esc_attr( $title ); ?>" href="<?php echo esc_url( $previous_post_permalink ); ?>"><?php echo esc_attr( $title ); ?></a></div>
				</div>
			</div>
			<?php
		endif;
...
/**
 * Helper function to get the comment pop-up links used in the theme's postmetadata block.
 * 
 */
function  udesign_get_comments_link() {
	...
	if ( ! post_password_required() && ( comments_open() ) ) {
		ob_start();
		?>
			<span class="postmetadata-comments-link"> / &nbsp; <?php comments_popup_link( __( 'Leave a comment', 'u-design' ), __( '1 Comment', 'u-design' ), __( '% Comments', 'u-design' ) ); ?></span>
		<?php
...
function udesign_footer_back_to_top_link() {
	global $udesign_options, $udesign_font_awesome_options;
	$angle_up_icon_source = ( ! $udesign_font_awesome_options['udesign_disable_font_awesome'] ) ? 'fa fa-angle-up': 'plain-angle-up';
	ob_start();
	?>
		<?php if ( 'angle-up' === $udesign_options['back_to_top'] ) : ?>
			<a href="#" id="back-to-top-fixed" style="display: none;"><span class="<?php echo esc_attr( $angle_up_icon_source ); ?>"></span><span class="back-to-top">Back to Top</span></a>
		<?php elseif ( 'text' === $udesign_options['back_to_top'] ) : ?>
			<div class="back-to-top">
				<a href="#top"><?php esc_html_e( 'Back to Top', 'u-design' ); ?></a>
			</div>
		<?php endif; ?>
	<?php
...
if ( $udesign_options['udesign_sticky_footer'] ) {
	function udesign_sticky_footer_func() {
		...
		echo wp_kses_post( $sticky_footer_html );
	}
	...
	function udesign_insert_wrapper1_closing_div() {
		...
		echo wp_kses_post( $wrapper1_closing_div );
	}
...
if( current_user_can( 'manage_options' ) && $udesign_options['show_udesign_action_hooks'] == 'yes' ) {
	...
	include( trailingslashit( get_template_directory() ) . 'inc/admin/u-design-hooks/u-design-show-action-hook-locations.php' );
}
...
/**
 * Set the flags for Max or Custom Width particular page overwriting 
 * global width with shortcodes (used to load "fluid.css" when required).
 * 
 * @global type $post
 */
function is_custom_width_page_func() {
	...
	// Override the global page/sidebar width option has been enabled for the current page.
	if ( $udesign_overwrite_page_width ) {
		/**
		 * Max Width Page: 
		 * Process custom field and set 'udesign_max_width_page' flag accordingly for a particular page.
		 */
		...
		if ( $udesign_max_page_width ) {
			...
			// Add 'udesign-is-custom-fluid-page' class to body tag.
			function udesign_add_is_custom_fluid_page_body_class( $classes ) {
				// Add 'udesign-is-custom-fluid-page' to the $classes array.
				$classes[] = 'udesign-is-custom-fluid-page';
				return $classes;
			}
			add_filter( 'body_class', 'udesign_add_is_custom_fluid_page_body_class' );
		}
		
		/**
		 * Custom Width Page: 
		 * Process custom fields and set 'udesign_custom_width_page' flag accordingly for a particular page.
		 */	
		...
		if ( is_numeric( $udesign_custom_page_width ) || is_numeric( $udesign_custom_sidebar_width ) ) { 
			...
			if ( ! $udesign_max_page_width ) {
				// Add 'udesign-is-custom-page-width' class to body tag.
				function udesign_add_is_custom_page_width_body_class( $classes ) {
					// Add 'udesign-is-custom-page-width' to the $classes array.
					$classes[] = 'udesign-is-custom-page-width';
					return $classes;
				}
				add_filter( 'body_class', 'udesign_add_is_custom_page_width_body_class' );
			}
		}
	} else {
		// If globally set fluid (max width) layout.
		if ( $udesign_options['max_theme_width'] ) {
			// Add 'udesign-is-global-fluid-page' class to body tag.
			function udesign_add_is_global_fluid_page_body_class( $classes ) {
				// Add 'udesign-is-global-fluid-page' to the $classes array.
				$classes[] = 'udesign-is-global-fluid-page';
				return $classes;
			}
			add_filter( 'body_class', 'udesign_add_is_global_fluid_page_body_class' );

		} else { // Globally set page width layout.
		
			// Add 'udesign-is-global-page-width' class to body tag.
			function udesign_add_is_global_page_width_body_class( $classes ) {
				// Add 'udesign-is-global-page-width' to the $classes array.
				$classes[] = 'udesign-is-global-page-width';
				return $classes;
			}
			add_filter( 'body_class', 'udesign_add_is_global_page_width_body_class' );
		}
...
function custom_width_page_func() {
	...
	if ( get_theme_mod( 'udesign_max_width_page' ) == 'yes' || get_theme_mod( 'udesign_custom_width_page' ) == 'yes' ) {
		...
		// If the received page width values are not sane then grab the theme's global page width option.
		if( ! is_numeric($udesign_custom_page_width ) ||  $udesign_custom_page_width < 960  ) {
		    $udesign_custom_page_width = 960; 
		}
		$udesign_custom_sidebar_width = get_post_meta( $post->ID, '_udesign_custom_sidebar_width', true );

		// If the received sidebar width values are not sane then grab the theme's global sidebar width option.
		...
		<style type="text/css">
			@media screen and (min-width: 960px) {
				...
				<?php else : ?>
					.container_24 {
						max-width: <?php echo wp_strip_all_tags( $udesign_custom_page_width ); ?>px;
						width: auto;
						margin-left: auto;
						margin-right: auto;
					}
					@media screen and (min-width: 1000px){ #feedback { display: block; } }
					@media screen and (max-width: <?php echo wp_strip_all_tags( $udesign_custom_page_width + 40 ); ?>px) { #feedback { display: none; } }
					@media screen and (min-width: 1040px){ #page-peel { display: block; } }
					@media screen and (max-width: <?php echo wp_strip_all_tags( $udesign_custom_page_width + 100 ); ?>px) { #page-peel { display: none; } }
					
					/* Handle Gutenberg's alignwide option for custom width page. */
					@media screen and (min-width: <?php echo wp_strip_all_tags( $udesign_custom_page_width + 140 ); ?>px) {
						.udesign-is-custom-page-width #main-content .alignwide {
							margin-left: -70px;
							margin-right: -70px;
							max-width: <?php echo wp_strip_all_tags( $udesign_custom_page_width + 100 ); ?>px;
						}
						...
						.udesign-is-custom-page-width #main-content .wp-block-pullquote.alignwide {
							width: <?php echo wp_strip_all_tags( $udesign_custom_page_width + 100 ); ?>px;
						}
					}
				<?php endif; ?>
				/* Sidebar. */
				#main-content.grid_16 { width: <?php echo wp_strip_all_tags( $udesign_custom_content_width ); ?>%; }
				#sidebar.grid_8 { width: <?php echo wp_strip_all_tags( $udesign_custom_sidebar_width ); ?>%; }
				#sidebar.push_8, #main-content.push_8 { left: <?php echo wp_strip_all_tags( $udesign_custom_sidebar_width ); ?>%; }
				#main-content.pull_16, #sidebar.pull_16 { right: <?php echo wp_strip_all_tags( $udesign_custom_content_width ); ?>%; }
			}
		</style>
		<?php
	}
...
function udesign_sitemap_page_content() {
    if ( is_page_template( 'sitemap.php' ) ) {
        ob_start(); ?>
                    <div class="one_half">
                            <h3><?php esc_html_e('Site Feeds', 'u-design'); ?></h3>
                            <ul class="list-10">
                                    <li><a href="<?php bloginfo('rss2_url'); ?>"><?php esc_html_e('Main RSS Feed', 'u-design'); ?></a></li>
                                    <li><a href="<?php bloginfo('comments_rss2_url'); ?>"><?php esc_html_e('Comments RSS Feed', 'u-design'); ?></a></li>
                            </ul>

                            <h3><?php esc_html_e('Pages', 'u-design'); ?></h3>
                            <ul class="list-10">
                                    <?php wp_list_pages('title_li='); ?>
                            </ul>

                            <h3><?php esc_html_e('Categories', 'u-design'); ?></h3>
                            <ul class="list-10">
                                    <?php wp_list_categories('title_li='); ?>
                            </ul>

                            <h3><?php esc_html_e('Monthly Archives', 'u-design'); ?></h3>
                            <ul class="list-10">
                                    <?php wp_get_archives('type=monthly'); ?>
                            </ul>
    <?php			if ( function_exists('wp_tag_cloud') ) : ?>
                                <h3><?php esc_html_e('Tags', 'u-design'); ?></h3>
    <?php			    echo preg_replace('/class=\"(.*?)\"|class=\'(.*?)\'/', 'class="list-10"', wp_tag_cloud('smallest=9&largest=9&format=list&echo=0'));
                            endif; ?>
                    </div>

                    <div class="one_half last_column">
                            <h3><?php esc_html_e('All Articles', 'u-design'); ?></h3>
                            <ol class="list-2">
    <?php			    $all_articles = new WP_Query('showposts=-1');
                                if ($all_articles->have_posts()) : 
                                    while ($all_articles->have_posts()) : $all_articles->the_post(); ?>
                                        <li style="margin-bottom:10px;"><a href="<?php the_permalink() ?>" rel="bookmark" title="Permanent Link to <?php the_title(); ?>"><?php the_title(); ?></a><br /><?php the_time(__('j-M-y', 'u-design')); ?> &bull; <?php the_author_posts_link(); ?> &bull; <?php comments_popup_link( __( '0 Comment', 'u-design' ), __( '1 Comment', 'u-design' ), __( '% Comments', 'u-design' ) ); ?></li>
    <?php                           endwhile;
...
if( is_admin() && class_exists( 'RevSliderAdmin' ) ) {
	...
	if( 'false' === $validated ) {
		...
		global $pagenow;
		if( $pagenow == 'plugins.php' && is_object( $GLOBALS['rs_admin'] ) ){
			remove_action( 'admin_notices', array( $GLOBALS['rs_admin'], 'add_plugins_page_notices' ) );
		}
	}
}
...
/**
 * U-Design Theme specific tweaks for Essential Grid plugin.
 * 
 */
if( is_admin() && class_exists( 'Essential_Grid_Admin' ) ) {
	...
	if( $validated === 'false' ){
		// Set the Ess. Grid plugin as theme. This hides the activation notice and the activation area in the Slider Overview.
		if ( function_exists( 'set_ess_grid_as_theme' ) ) {
			set_ess_grid_as_theme();
		}
		// Remove eg's nag.
		$eg_instance = Essential_Grid_Admin::get_instance();
		if( is_object( $eg_instance ) ) {
			remove_action( 'admin_notices', array( $eg_instance, 'add_activate_notification' ) );
		}
	}
}
...
/**
 * This fixes the issue where Revolution slider takes ~ 10 minutes to install/activate.
 * The reason being that upon initial activation the plugin loads the entire "Templates" library which could take a long time.
 * Once the "Templates" library has been loaded a flag is set in the options table. 
 * In the function below all we're doing is setting that flag upon the plugin installation.
 * This will not interfere in any way with the "Templates" library functionality which could be accessed through "Slider Revolution -> Add Slider From Template".
 * 
 */
function udesign_revslider_install_fix ( $bool, $args_hook_extra, $result ) {
	
	if ( 'revslider' === $result['destination_name'] ) {
		update_option( 'revslider-templates-check',  time() );
	}
	return $bool;
}
add_filter( 'upgrader_post_install', 'udesign_revslider_install_fix', 10, 3 );



// Add U-Design own elements/shortcodes to VC
require_once( trailingslashit( get_template_directory() ) . 'inc/admin/vc-elements/u-design-vc-elements.php' ); 
...
function udesign_add_custom_editor_styles() {
	...
	if ( ! $udesign_font_awesome_options['udesign_disable_font_awesome'] ) {
		add_editor_style( get_template_directory_uri() . '/inc/shared/font-awesome/css/font-awesome.min.css' );
	}
}
add_action( 'admin_enqueue_scripts', 'udesign_add_custom_editor_styles' );
			...
			<nav id="secondary-navigation-bar-wrapper">
				<div id="secondary-navigation-bar" class="container_24">
					<div id="secondary-navigation-bar-content">
						<?php
						if ( $udesign_options['secondary_menu_text_area_1_width'] != 0 ) :
							ob_start();
							?>
							<div id="secondary-nav-bar-location-1" class="grid_<?php echo esc_attr( $udesign_options['secondary_menu_text_area_1_width'] ); ?>">
								<div id="sec-nav-text-area-1">
									<?php echo do_shortcode( __( $udesign_options['secondary_menu_text_area_1'], 'u-design' ) ); ?>
								</div>
							</div>
							<?php
							$text_1_html = ob_get_clean();
						endif;
						if ( $udesign_options['secondary_menu_text_area_2_width'] != 0 ) :
							ob_start();
							?>
							<div id="secondary-nav-bar-location-2" class="grid_<?php echo esc_attr( $udesign_options['secondary_menu_text_area_2_width'] ); ?>">
								<div id="sec-nav-text-area-2">
									<?php echo do_shortcode( __( $udesign_options['secondary_menu_text_area_2'], 'u-design' ) ); ?>
								</div>
							</div>
							<?php
							$text_2_html = ob_get_clean();
						endif;
						if ( $udesign_options['secondary_menu_width'] != 0 ) :
							ob_start();
							?>
							<div id="secondary-nav-bar-location-3" class="grid_<?php echo esc_attr( $udesign_options['secondary_menu_width'] ); ?>">
								<div id="sec-nav-menu-area">
									<?php
									if ( is_numeric($secondary_menu_id) ) {
						...
						switch ( $udesign_options['secondary_menu_items_order'] ) {
							case "txt1|menu|txt2":
								echo wp_kses_post( $text_1_html . $secondary_menu_html . $text_2_html );
								break;
							case "txt1|txt2|menu":
								echo wp_kses_post( $text_1_html . $text_2_html . $secondary_menu_html );
								break;
							case "menu|txt1|txt2":
								echo wp_kses_post( $secondary_menu_html . $text_1_html . $text_2_html );
								break;
							case "menu|txt2|txt1":
								echo wp_kses_post( $secondary_menu_html . $text_2_html . $text_1_html );
								break;
							case "txt2|menu|txt1":
								echo wp_kses_post( $text_2_html . $secondary_menu_html . $text_1_html );
								break;
							case "txt2|txt1|menu": 
								echo wp_kses_post( $text_2_html . $text_1_html . $secondary_menu_html );
								break;
							case "txt1|menu":
								echo wp_kses_post( $text_1_html . $secondary_menu_html );
								break;
							case "menu|txt1":
								echo wp_kses_post( $secondary_menu_html . $text_1_html );
								break;
							case "txt2|menu":
								echo wp_kses_post( $text_2_html . $secondary_menu_html );
								break;
							case "menu|txt2":
								echo wp_kses_post( $secondary_menu_html . $text_2_html );
								break;
							case "txt1|txt2":
								echo wp_kses_post( $text_1_html . $text_2_html );
								break;
							case "txt2|txt1":
								echo wp_kses_post( $text_2_html . $text_1_html );
								break;
							case "txt1":
								echo wp_kses_post( $text_1_html );
								break;
							case "txt2":
								echo wp_kses_post( $text_2_html );
								break;
							case "menu":
								echo wp_kses_post( $secondary_menu_html );
								break;
...
function udesign_add_admin_bar() {
	...
	$args = array(
		...
		'title'  => __( 'U-Design Options', 'u-design' ),
	...
	$args = array(
		...
		'parent' => 'u-design-settings-page',
		'title'  => __( 'Theme Update', 'u-design' ),
	...
	$args = array(
		...
		'parent' => 'u-design-settings-page',
		'title'  => __( 'Backup / Restore', 'u-design' ),
	...
	$wp_admin_bar->add_node( $args );
        ...
        if ( class_exists( 'Merlin' ) ) {
            $args = array(
                    'id'     => 'u-design-demos-import-child',
                    'parent' => 'u-design-settings-page',
                    'title'  => __( 'Import Demo Data', 'u-design' ),
                    'href'   => admin_url('admin.php?page=udesign_demo_import'),
            );
            $wp_admin_bar->add_node( $args );
        }
...
/**
 * Load CMB2 library only when WordPress admin area and 
 * only where post/page editing is required, eg. post, page and custom post type.
 * 
 */
global $pagenow;
if (  is_admin() && ( ( 'post-new.php' === $pagenow || 'post.php' === $pagenow ) ) ) {
	include( trailingslashit( get_template_directory() ) . 'inc/admin/u-design-cmb2/u-design-cmb2-functions.php' );
}
...
function udesign_style_comment_fields( $fields ) {
    ...
    $fields_new =  array(
            'author' => '<p class="comment-form-author">'.
                        ...
                        '<label for="author">' . esc_html__('name', 'u-design') . ( $req ? ' <span class="required">' . esc_html__('(required)', 'u-design') . '</span>' : '' ) . '</label>' . 
                        '</p>',
            'email'  => '<p class="comment-form-email">' .
                        ...
                        '<label for="email">' . esc_html__('email', 'u-design') . ' <span class="will-not-be-published">' . esc_html__('(will not be published)', 'u-design'). '</span>' . ( $req ? ' <span class="required">' . esc_html__('(required)', 'u-design') . '</span>' : '' ) . '</label> ' . 
                        '</p>',
            'url'    => '<p class="comment-form-url">'.
                        '<input id="url" name="url" type="text" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" /> ' . 
                        '<label for="url">' . esc_html__('website', 'u-design') . '</label>' . 
                        '</p>',
    );
    return array_merge( $fields, $fields_new );
}
...
/*
 * The Following will output a notice to users to let them know that they need to save the theme's Settings page after updating the theme post version 3.0.0
 * 
 */
function udesign_check_theme_version_for_admin_notices() {
    global $udesign_options;
    if ( current_user_can( who_can_edit_udesign_theme_options() ) && version_compare(UDESIGN_VERSION, '3.0.0', '>') && (  !empty( $udesign_options ) && version_compare( $udesign_options['udesign_settings_page_last_saved_version'], '3.0.0', '<=' ) ) ) {

        function udesign_update_theme_settings_admin_notice() { ?>
                <div class="notice notice-warning is-dismissible">
                    <p><strong>IMPORTANT MESSAGE (U-DESIGN THEME):</strong> Looks like you just updated the U-Design theme. Some of the new features require that you <strong>save</strong> the <a href="<?php echo esc_url(admin_url( 'admin.php' ).'?page=udesign_options_page'); ?>">U-Design Settings page</a>!</p>
                </div>
            <?php 
        }
	...
	if ( $post_id ) {
		...
		if ( $attachment_id ) : 
			...
			<div id="page-custom-header">
				...
				<?php if ( $top_area_over_header_image ) : ?>
					<?php 
					...
					<script type="text/javascript">
						...
						if ( topWrapper != null ) {
							...
							jQuery(window).load(function () {
								var respMenu2AtBrPoint1 = '<?php echo esc_attr( $resp_menu_2_at_br_point_1 ); ?>';
					...
					<style>@media screen and (min-width: 720px){#top-wrapper{background-color:<?php echo wp_strip_all_tags( $page_top_area_bg ); ?>;background-image: none;position:relative;}}@media screen and (max-width: 719px){#page-custom-header img{margin-top:0 !important;}}</style>
				<?php endif; ?>
...
<p>To begin please make sure to first review the theme's <a target="_blank" href="<?php echo trailingslashit( get_template_directory_uri() ) . 'inc/shared/documentation/index.html'; ?>">documentation</a>.</p>
...
if ( ! function_exists( 'udesign_gutenberg_setup' ) ) {
	/**
	 * Sets up U-Design theme support for Gutenberg editor.
	 */
	function udesign_gutenberg_setup() {
		...
		// Add support for editor styles.
		add_editor_style( get_template_directory_uri() . '/assets/css/admin/custom-editor-style.php' );
		
		// Add custom editor font sizes.
		add_theme_support( 'editor-font-sizes', array (
				array(
					'name' => __( 'small', 'u-design' ),
					'shortName' => __( 'S', 'u-design' ),
					'size' => $udesign_options['general_font_size'] - 2,
					'slug' => 'small'
				),
				array(
					'name' => __( 'normal', 'u-design' ),
					'shortName' => __( 'N', 'u-design' ),
					'size' => $udesign_options['general_font_size'],
					'slug' => 'normal'
				),
				array(
					'name' => __( 'medium', 'u-design' ),
					'shortName' => __( 'M', 'u-design' ),
					'size' => $udesign_options['general_font_size'] + 4,
					'slug' => 'medium'
				),
				array(
					'name' => __( 'large', 'u-design' ),
					'shortName' => __( 'L', 'u-design' ),
					'size' => $udesign_options['general_font_size'] + 20,
					'slug' => 'large'
				),
				array(
					'name' => __( 'huge', 'u-design' ),
					'shortName' => __( 'XL', 'u-design' ),
					'size' => $udesign_options['general_font_size'] + 34,
					'slug' => 'larger'
				)
			)
		);
...
add_action( 'after_setup_theme', 'udesign_gutenberg_setup' );
 ...
 * 
*/
function udesign_gutenberg_editor_styles() {
	...
	// Enqueue Gutenberg specific styles.
	wp_enqueue_style( 'u-design-block-editor-styles', get_template_directory_uri() . '/assets/css/admin/gutenberg-editor-styles.css', array( 'wp-edit-post' ) );
	
}
add_action( 'enqueue_block_editor_assets', 'udesign_gutenberg_editor_styles' );


/**
 * Prevent WooCommerce automatic wizard redirect.
 * Otherwise it interrupts with TGMPA as well as theme's wizard import process.
 * 
 */
function udesign_prevent_wc_automatic_wizard_redirect() {
	
	add_filter( 'woocommerce_prevent_automatic_wizard_redirect', '__return_true' );
	
}
add_action( 'after_setup_theme', 'udesign_prevent_wc_automatic_wizard_redirect' );


/*
 * The Following will output a notice to admin users to let them know that the 
 * "Statistics" section will be removed within a few months from May 31st, 2019.
 * 
 */
if ( current_user_can( who_can_edit_udesign_theme_options() ) ) {
    
	function udesign_new_requirements_admin_notice() {
		global $current_user;
		$user_id = $current_user->ID;
		$screen = get_current_screen();
		
		if( 'udesign_options_page' === $screen->parent_base  && ! get_user_meta( $user_id, 'udesign_new_requirements_notice_ignore' ) ) :
			?>
			<div class="notice notice-warning is-dismissible">
				<p><strong>U-DESIGN NOTICE:</strong> As per the new Envato requirements, themes containing plugin territory functionality such as analytics code, 
				    must move it to a plugin or remove it from the theme. The <a href="<?php echo esc_url(admin_url( 'admin.php' ).'?page=udesign_options_page'); ?>">U-Design Settings page</a> contains a section called "Statistics" which will be removed from the theme within a few months from May 31st, 2019.
				    If you're using this section it would be in your interest to move your Google Analytics or other code to avoid disruption in your stats collection. You may move it to a third party plugin that's intended for such use or use <a href="https://dreamthemedesign.com/u-design-support/discussion/32792/statistics-section-will-be-removed-from-the-theme-heres-how-to-add-google-analytics-or-other-code" target="_blank">the following method</a>.
				    Sorry for the inconvenience. 
				</p>
				<?php
				$ignore_notice_url = add_query_arg(
					array(
						'page' => 'udesign_options_page',
						'udesign-new-requirements-ignore-notice' => 'true'
					), 
					admin_url( 'admin.php' ) 
				);
				?>
				<p><a href="<?php echo esc_url( wp_nonce_url( $ignore_notice_url, 'udesign-new-req-ignore-notice-action', 'udesign-new-req-ignore-notice-nonce' ) ); ?>">Dismiss this notice</a></p>
			</div>
			<?php
		endif;
	}
	add_action( 'admin_notices', 'udesign_new_requirements_admin_notice' );
	
	function udesign_new_requirements_notice_ignore_save() {
		global $current_user;
		$user_id = $current_user->ID;
		if ( isset( $_GET['udesign-new-requirements-ignore-notice'] ) ) {
			// Check security.
			check_admin_referer( 'udesign-new-req-ignore-notice-action', 'udesign-new-req-ignore-notice-nonce' );
			// Save the user's dismiss message action.
			add_user_meta( $user_id, 'udesign_new_requirements_notice_ignore', 'true', true );
			// Send the user back to where they came from.
			$redirect_to = isset( $_SERVER['HTTP_REFERER'] ) ? $_SERVER['HTTP_REFERER'] : admin_url( 'admin.php?page=udesign_options_page' );
			wp_safe_redirect( $redirect_to );
		}
	}
	add_action( 'admin_init', 'udesign_new_requirements_notice_ignore_save' );
